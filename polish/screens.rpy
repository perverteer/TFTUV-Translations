# TODO: Translation updated at 2020-06-22 15:18

translate polish strings:

    # game/screens.rpy:279
    old "Back"
    new "Cofnij"

    # game/screens.rpy:280
    old "History"
    new "Historia"

    # game/screens.rpy:281
    old "Skip"
    new "Pomiń"

    # game/screens.rpy:282
    old "Auto"
    new "Auto"

    # game/screens.rpy:283
    old "Save"
    new "Zapisz"

    # game/screens.rpy:284
    old "Q.Save"
    new "Sz.zapis"

    # game/screens.rpy:285
    old "Q.Load"
    new "Sz.odczyt"

    # game/screens.rpy:287
    old "Codex"
    new "Kodeks"

    # game/screens.rpy:288
    old "Prefs"
    new "Ustawienia"

    # game/screens.rpy:347
    old "Start"
    new "Start"

    # game/screens.rpy:355
    old "Load"
    new "Wczytaj"

    # game/screens.rpy:360
    old "Preferences"
    new "Ustawienia"

    # game/screens.rpy:364
    old "End Replay"
    new "Zakończ Powtórkę"

    # game/screens.rpy:367
    old "Scene Gallery"
    new "Galeria Scen"

    # game/screens.rpy:371
    old "Main Menu"
    new "Menu Główne"

    # game/screens.rpy:373
    old "About"
    new "O..."

    # game/screens.rpy:378
    old "Help"
    new "Pomoc"

    # game/screens.rpy:381
    old "Quit"
    new "Zakończ"

    # game/screens.rpy:597
    old "Return"
    new "Powrót"

    # game/screens.rpy:725
    old "Extras"
    new "Dodatki"

    # game/screens.rpy:727
    old "Ep"
    new "Ep"

    # game/screens.rpy:773
    old "Version "
    new "Wersja"

    # game/screens.rpy:773
    old "[config.version!t]\n"
    new "[config.version!t]\n"

    # game/screens.rpy:779
    old "Music"
    new "Muzyka"

    # game/screens.rpy:796
    old "{a=[track[website]]}[track[website]]{/a}"
    new "{a=[track[website]]}[track[website]]{/a}"

    # game/screens.rpy:797
    old "License: "
    new "Licencja: "

    # game/screens.rpy:797
    old "{a=[track[license_url]]}[track[license]]{/a}"
    new "{a=[track[license_url]]}[track[license]]{/a}"

    # game/screens.rpy:801
    old "{size=-8}Uses portions of the {a=https://github.com/jsfehler/renpy-encyclopaedia}Encyclopaedia Framework for Ren'Py{/a} by jsfehler"
    new "{size=-8}Używa fragmentów {a=https://github.com/jsfehler/renpy-encyclopaedia}Encyclopaedia Framework for Ren'Py{/a} od jsfehler"

    # game/screens.rpy:802
    old "{size=-8}Ashtar Flying Disk, Federation Interceptor HN48, Endor Battlecruiser, Sky Ranger Domminator, Titan Class II Cargo Ship, Wraith Raider Starship models created by {a=https://www.renderosity.com/?uid=767619}sevein/Herminio Nieves{/a}"
    new "{size=-8} Modele Ashtar Flying Disk, Federation Interceptor HN48, Endor Battlecruiser, Sky Ranger Domminator, Titan Class II Cargo Ship, Wraith Raider Starship stworzone przez {a=https://www.renderosity.com/?uid=767619}sevein/Herminio Nieves{/a}"

    # game/screens.rpy:803
    old "{size=-8}Crucero Space Army model created by {a=http://www.sharecg.com/pf/full_uploads.php?pf_user_name=thomasjeromenewton}Antonio Amador{/a}"
    new "{size=-8}Model Crucero Space Army stworzony przez {a=http://www.sharecg.com/pf/full_uploads.php?pf_user_name=thomasjeromenewton}Antonio Amador{/a}"

    # game/screens.rpy:804
    old "{size=-8}Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "{size=-8}Wykonano z {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"

    # game/screens.rpy:843
    old "Page {}"
    new "Strona {}"

    # game/screens.rpy:843
    old "Automatic saves"
    new "Auto zapisy"

    # game/screens.rpy:843
    old "Quick saves"
    new "Szybkie zapisy"

    # game/screens.rpy:892
    old "{#file_time}%A, %B %d %Y, %H:%M"
    new "{#file_time}%A, %B %d %Y, %H:%M"

    # game/screens.rpy:892
    old "empty slot"
    new "puste miejsce"

    # game/screens.rpy:915
    old "<"
    new "<"

    # game/screens.rpy:918
    old "{#auto_page}A"
    new "{#auto_page}A"

    # game/screens.rpy:921
    old "{#quick_page}Q"
    new "{#quick_page}Q"

    # game/screens.rpy:927
    old ">"
    new ">"

    # game/screens.rpy:977
    old "Please name your save:"
    new "Proszę nazwać zapis:"

    # game/screens.rpy:988
    old "Save game"
    new "Zapisz grę"

    # game/screens.rpy:1019
    old "Player Names"
    new "Nazwij gracza"

    # game/screens.rpy:1020
    old "Name: "
    new "Imię:"

    # game/screens.rpy:1021
    old "Nickname: "
    new "Przydomek:"

    # game/screens.rpy:1027
    old "Display"
    new "Wyświetlanie"

    # game/screens.rpy:1028
    old "Window"
    new "Okno"

    # game/screens.rpy:1029
    old "Fullscreen"
    new "Pełny ekran"

    # game/screens.rpy:1033
    old "Rollback Side"
    new "Strona Wycofywania"

    # game/screens.rpy:1034
    old "Disable"
    new "Wyłącz"

    # game/screens.rpy:1035
    old "Left"
    new "Lewa"

    # game/screens.rpy:1036
    old "Right"
    new "Prawa"

    # game/screens.rpy:1041
    old "Unseen Text"
    new "Niewidziany tekst"

    # game/screens.rpy:1042
    old "After Choices"
    new "Po Wyborze"

    # game/screens.rpy:1043
    old "Transitions"
    new "Przejścia"

    # game/screens.rpy:1056
    old "Text Speed"
    new "Szybkość Tekstu"

    # game/screens.rpy:1060
    old "Auto-Forward Time"
    new "Czas szybkiego naprzód"

    # game/screens.rpy:1067
    old "Music Volume"
    new "Głośność Muzyki"

    # game/screens.rpy:1074
    old "Sound Volume"
    new "Głośność Dźwięku"

    # game/screens.rpy:1080
    old "Test"
    new "Test"

    # game/screens.rpy:1084
    old "Voice Volume"
    new "Głośność Głosu"

    # game/screens.rpy:1095
    old "Mute All"
    new "Wycisz Wszystko"

    # game/screens.rpy:1217
    old "The dialogue history is empty."
    new "Historia dialogów jest pusta"

    # game/screens.rpy:1287
    old "Keyboard"
    new "Klawiatura"

    # game/screens.rpy:1288
    old "Mouse"
    new "Mysz"

    # game/screens.rpy:1291
    old "Gamepad"
    new "Gamepad"

    # game/screens.rpy:1304
    old "Enter"
    new "Enter"

    # game/screens.rpy:1305
    old "Advances dialogue and activates the interface."
    new "Postęp dialogu i aktywacja interfejsu."

    # game/screens.rpy:1308
    old "Space"
    new "Spacja"

    # game/screens.rpy:1309
    old "Advances dialogue without selecting choices."
    new "Postęp dialogu bez zaznaczenia wyboru."

    # game/screens.rpy:1312
    old "Arrow Keys"
    new "Klawisze Strzałek"

    # game/screens.rpy:1313
    old "Navigate the interface."
    new "Nawiguj w interfejsie."

    # game/screens.rpy:1316
    old "Escape"
    new "Escape"

    # game/screens.rpy:1317
    old "Accesses the game menu."
    new "Wejdź do menu gry."

    # game/screens.rpy:1320
    old "Ctrl"
    new "Ctrl"

    # game/screens.rpy:1321
    old "Skips dialogue while held down."
    new "Przytrzymany pomija dialogi."

    # game/screens.rpy:1324
    old "Tab"
    new "Tab"

    # game/screens.rpy:1325
    old "Toggles dialogue skipping."
    new "Przełancza pomijanie dialogu."

    # game/screens.rpy:1328
    old "Page Up"
    new "Page Up"

    # game/screens.rpy:1329
    old "Rolls back to earlier dialogue."
    new "Przewija do wcześniejszego dialogu."

    # game/screens.rpy:1332
    old "Page Down"
    new "Page Down"

    # game/screens.rpy:1333
    old "Rolls forward to later dialogue."
    new "Przewija do następnego dialogu."

    # game/screens.rpy:1337
    old "Hides the user interface."
    new "Ukrywa interfejs użytkownika."

    # game/screens.rpy:1341
    old "Takes a screenshot."
    new "Robi zrzut ekranowy."

    # game/screens.rpy:1345
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new "Przełącza asystowanie {a=https://www.renpy.org/l/voicing}self-voicing{/a}."

    # game/screens.rpy:1351
    old "Left Click"
    new "Lewe kliknięcie"

    # game/screens.rpy:1355
    old "Middle Click"
    new "Środkowe kliknięcie"

    # game/screens.rpy:1359
    old "Right Click"
    new "Prawe kliknięcie"

    # game/screens.rpy:1363
    old "Mouse Wheel Up\nClick Rollback Side"
    new "Kółko myszy w górę\nKliknięcie Strony Wycofywania"

    # game/screens.rpy:1367
    old "Mouse Wheel Down"
    new "Kółko myszy w dół"

    # game/screens.rpy:1374
    old "Right Trigger\nA/Bottom Button"
    new "Prawy Spust\nA/Dolny Przycisk"

    # game/screens.rpy:1378
    old "Left Trigger\nLeft Shoulder"
    new "Lewy Spust\nLeft Shoulder"

    # game/screens.rpy:1382
    old "Right Shoulder"
    new "Right Shoulder"

    # game/screens.rpy:1387
    old "D-Pad, Sticks"
    new "D-Pad, Sticks"

    # game/screens.rpy:1391
    old "Start, Guide"
    new "Start, Przewodnik"

    # game/screens.rpy:1395
    old "Y/Top Button"
    new "Y/Top Button"

    # game/screens.rpy:1398
    old "Calibrate"
    new "Kalibracja"

    # game/screens.rpy:1463
    old "Yes"
    new "Tak"

    # game/screens.rpy:1464
    old "No"
    new "Nie"

    # game/screens.rpy:1511
    old "Skipping"
    new "Pomijanie"

    # game/screens.rpy:1734
    old "Menu"
    new "Menu"

    # game/screens.rpy:1735
    old "Hide"
    new "Ukryj"

# TODO: Translation updated at 2020-08-17 11:51

translate polish strings:

    # game/screens.rpy:768
    old "Relationships"
    new "Relacje z osobami"

    # game/screens.rpy:774
    old "Close"
    new "Zamknij"

    # game/screens.rpy:806
    old "Relationship: %s"
    new "Związek:% s"

    # game/screens.rpy:808
    old "Pregnant: %s"
    new "W ciąży: %s"

    # game/screens.rpy:972
    old "{size=-8}Ashtar Flying Disk, Federation Interceptor HN48, Endor Battlecruiser, Sky Ranger Domminator, Titan Class II Cargo Ship, Wraith Raider Starship, Eagle 5 Transport models created by {a=https://www.renderosity.com/?uid=767619}sevein/Herminio Nieves{/a}"
    new "{size=-8}Ashtar Flying Disk, Federation Interceptor HN48, Endor Battlecruiser, Sky Ranger Domminator, Titan Class II Cargo Ship, Wraith Raider Starship, Eagle 5 Transport modele stworzone przez {a=https://www.renderosity.com/?uid = 767619}sevein/ Herminio Nieves{/a}"

    # game/screens.rpy:1220
    old "Language"
    new "Język"

    # game/screens.rpy:1221
    old "English"
    new "Angielski"

    # game/screens.rpy:1222
    old "Russian"
    new "Rosyjski"

    # game/screens.rpy:1227
    old "Notifications"
    new "Powiadomienia"

    # game/screens.rpy:1228
    old "Show"
    new "Pokaż"

    # game/screens.rpy:1253
    old "Dialog Box Visibility"
    new "Widoczność okna dialogowego"

    # game/screens.rpy:1542
    old "Shows the relationships screen"
    new "Wyświetla ekran relacji z postacią."

    # game/screens.rpy:1546
    old "Shows the codex"
    new "Wyświetla kodeks."

# TODO: Translation updated at 2020-11-23 14:29

translate polish strings:

    # game/screens.rpy:1224
    old "Russian (partial)"
    new "Rosyjski (częściowo)"

    # game/screens.rpy:1225
    old "Polish (partial)"
    new "Polski (częściowo)"

    # game/screens.rpy:1226
    old "Italian (partial)"
    new "Włoski (częściowo)"

    # game/screens.rpy:1242
    old "Show Portraits"
    new "Pokaż Portrety"

# TODO: Translation updated at 2021-02-15 09:57

translate polish strings:

    # game/screens.rpy:973
    old "{size=-8}Ashtar Flying Disk, Federation Interceptor HN48, Endor Battlecruiser, Sky Ranger Dominator, Titan Class II Cargo Ship, Wraith Raider Starship, Eagle 5 Transport models created by {a=https://www.renderosity.com/?uid=767619}sevein/Herminio Nieves{/a}"
    new "{size=-8}Modele Ashtar Flying Disk, Federation Interceptor HN48, Endor Battlecruiser, Sky Ranger Dominator, Titan Class II Cargo Ship, Wraith Raider Starship, Eagle 5 Transport stworzone przez {a=https://www.renderosity.com/?uid=767619}sevein/Herminio Nieves{/a}"

    # game/screens.rpy:1227
    old "Portuguese"
    new "Portugalski"

# TODO: Translation updated at 2021-07-12 09:34

translate polish strings:

    # game/screens.rpy:363
    old "Continue"
    new "Kontynuuj"

# TODO: Translation updated at 2022-06-28 20:50

translate polish strings:

    # game/screens.rpy:386
    old "Achievements"
    new ""

    # game/screens.rpy:389
    old "Bonus"
    new ""

    # game/screens.rpy:1008
    old "Achievements Unlocked: %d / %d"
    new ""

    # game/screens.rpy:1047
    old "Translations"
    new ""

    # game/screens.rpy:1076
    old "{size=-8}Crucero Space Army and Space Gate models created by {a=http://www.sharecg.com/pf/full_uploads.php?pf_user_name=thomasjeromenewton}Antonio Amador{/a}"
    new ""

    # game/screens.rpy:1151
    old "Jump to page: "
    new ""

    # game/screens.rpy:1367
    old "Italian"
    new ""

    # game/screens.rpy:1371
    old "Spanish"
    new ""

    # game/screens.rpy:1385
    old "Incest"
    new ""

# TODO: Translation updated at 2022-07-27 15:35

translate polish strings:

    # game/screens.rpy:1375
    old "Turkish"
    new ""

# TODO: Translation updated at 2023-04-17 12:14

translate polish strings:

    # game/screens.rpy:899
    old "View by: "
    new ""

    # game/screens.rpy:901
    old "character"
    new ""

    # game/screens.rpy:903
    old "episode"
    new ""

    # game/screens.rpy:1112
    old "Website: "
    new ""

    # game/screens.rpy:1112
    old "{a=[track[composer_website]]}[track[composer_website]]{/a}"
    new ""

