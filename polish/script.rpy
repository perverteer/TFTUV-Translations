# TODO: Translation updated at 2020-06-22 15:18

translate polish strings:

    # game/script.rpy:116
    old "Confirm"
    new "Potwierdź"

# TODO: Translation updated at 2020-08-17 11:51

translate polish strings:

    # game/script.rpy:16
    old "Taboo Edition disabled"
    new "Edycja Taboo wyłączona"

    # game/script.rpy:18
    old "Taboo Edition enabled"
    new "Włączono Edycję Taboo"

    # game/script.rpy:38
    old "CODEX UPDATED"
    new "AKTUALIZACJA KODEKSU"

    # game/script.rpy:272
    old "Unread"
    new "Nieprzeczytane"

    # game/script.rpy:275
    old "What is your name?"
    new "Jak się nazywasz?"

    # game/script.rpy:278
    old "What is your nickname?"
    new "Jaki jest twój pseudonim?"

# TODO: Translation updated at 2021-07-12 09:34

translate polish strings:

    # game/script.rpy:183
    old "Please select the Season 1 save you'd like to load:"
    new "Proszę wybrać który zapis Sezonu 1, chcesz załadować:"

    # game/script.rpy:204
    old "Exit"
    new "Wyjście"

    # game/script.rpy:315
    old "No previous season data found"
    new "Nie znaleziono danych z poprzedniego sezonu"

    # game/script.rpy:320
    old "Please complete the previous season of the game before contuining with this version and make sure a Season save file is present in the saves directory."
    new "Proszę ukończyć poprzedni sezon gry przed rozpoczęciem tej wersji i upewnić się, że plik zapisu sezonu jest obecny w katalogu save'ów."

    # game/script.rpy:324
    old "Consult the game manual or {a=https://perverteer.com/faq}the FAQ section{/a} for more information."
    new "Więcej informacji można znaleźć w instrukcji gry, lub w FAQ {a=https://perverteer.com/faq}{/a}."

# TODO: Translation updated at 2021-10-13 09:38

translate polish strings:

    # game/script.rpy:320
    old "Please complete the previous season of the game before continuing with this version and make sure a Season save file is present in the saves directory."
    new "Proszę ukończyć poprzedni sezon gry przed kontynuowaniem w tej wersji i upewnić się, że plik zapisu sezonu jest obecny w katalogu save'ów."

# TODO: Translation updated at 2022-06-28 20:50

translate polish strings:

    # game/script.rpy:129
    old "ACHIEVEMENT UNLOCKED"
    new ""

    # game/script.rpy:413
    old "Story, artwork and development"
    new ""

    # game/script.rpy:413
    old "Perverteer"
    new ""

    # game/script.rpy:413
    old "Animations"
    new ""

    # game/script.rpy:413
    old "Alorth"
    new ""

    # game/script.rpy:413
    old "Proofreading"
    new ""

    # game/script.rpy:413
    old "Boerke, Dreamweaver, FckThisShtImOut, Freki, Fhilb, Grubb, marcurion, Razielprince and ThunderRob"
    new ""

    # game/script.rpy:413
    old "Testing"
    new ""

    # game/script.rpy:413
    old "HoboCop86, ThunderRob, Fhilb, Boerke, Dreamweaver, Alorth, marcurion, gamersglory, GhostPhil, Grubb, RavenDarklock, Freki and Razielprince"
    new ""

    # game/script.rpy:413
    old "Translation"
    new ""

    # game/script.rpy:413
    old "Grubb {i}(Portuguese){/i}, MyPrettyWolfy {i}(Russian){/i}, Freeman {i}(Polish){/i}, Frank Rosario {i}(Italian){/i}, Juan Manuel and Kabezon {i}(Spanish){/i}"
    new ""

    # game/script.rpy:413
    old "Special thanks to"
    new ""

    # game/script.rpy:413
    old "Alorth, Grubb, HoboCop86, th66, Mac and Morbil."
    new ""

    # game/script.rpy:413
    old "Very special thanks to"
    new ""

    # game/script.rpy:413
    old "All loyal patrons who generously supported the development of this project."
    new ""

# TODO: Translation updated at 2022-07-27 15:35

translate polish strings:

    # game/script.rpy:197
    old "Press {i}\"Enter\"{/i} on the on-screen keyboard to confirm"
    new ""

